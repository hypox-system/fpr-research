...
Exit when price rises above trailing stop level.
"""
...
@register_signal
@dataclass
class ATRStopShort(SignalComponent):
    """
    ATR trailing stop for short positions.

    Maintains a trailing stop at close + ATR * multiplier.
    Exit when close rises above trailing stop.
    """

    KEY: ClassVar[str] = 'atr_stop_short'
    ...
    def compute(self, df: pd.DataFrame, cache) -> pd.Series:
        ...
        # Trailing stop level (above price for shorts)
        stop_distance = atr * atr_mult
        stop_level = close + stop_distance

        # Trailing: stop can only move down for shorts
        trailing_stop = stop_level.cummin()
