...
def compute_fold_metrics(trade_returns: np.ndarray) -> Dict[str, float]:
    ...
    # Sharpe (annualized, assuming ~252 trading days, ~6 trades/day avg)
    mean_ret = np.mean(trade_returns)
    std_ret = np.std(trade_returns)
    sharpe = mean_ret / std_ret * np.sqrt(252 * 6) if std_ret > 0 else 0.0
...
def walk_forward_run(
    ...
    Args:
        ...
        config: Walk-forward configuration
        cache: FeatureStore for indicator computation
    ...
    for fold_id, (train_df, test_df) in enumerate(splits):
        ...
        try:
            ...
            pass  # IS diagnostic is optional
